...
Solution Heuristiques::methode_Heuristique_V1() {
	...
	for (int num_Jour = 0; num_Jour < nb_Jour; num_Jour++) {
		...
		for (int id_POI = 0; id_POI < nb_POI; id_POI++) {
			...
			// On peut ajouter le POI si en le choisissant on peut à un hôtel par la suite, qu'il n'a pas déjà été visité et que la  date de départ par défaut est possible.
